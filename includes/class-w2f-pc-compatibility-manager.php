 ...
 * This class maintains backward compatibility with the old rules-based system
 * while delegating to the new W2F_PC_Compatibility_Engine for evaluation.
 *
 * @package  W2F_PC_Configurator
 * @since    1.0.0
 * @deprecated 2.0.0 Use W2F_PC_Compatibility_Engine instead.
 */
 ...
 * @version  2.0.0
 * @deprecated Use W2F_PC_Compatibility_Engine instead.
 */
class W2F_PC_Compatibility_Manager {
	...
	/**
	 * Whether to use the new engine for compatibility checks.
	 *
	 * @var bool
	 */
	private $use_new_engine = true;
	...
	public function __construct() {
		...
		// Allow disabling new engine via filter for debugging.
		$this->use_new_engine = apply_filters( 'w2f_pc_use_new_compatibility_engine', true );
	}
	 ...
	 *
	 * This method now delegates to the new W2F_PC_Compatibility_Engine.
	 *
	...
	public function check_compatibility( $configuration, $configurator_product = null ) {
		// Use the new compatibility engine if enabled.
		if ( $this->use_new_engine && class_exists( 'W2F_PC_Compatibility_Engine' ) ) {
			$engine = W2F_PC_Compatibility_Engine::instance();
			$result = $engine->check_compatibility( $configuration, $configurator_product );

			// Remove trace from result for backward compatibility.
			unset( $result['trace'] );
	...
	public function get_filtered_products_for_component( $component_id, $configuration, $configurator_product = null ) {
		...
		if ( $this->use_new_engine && class_exists( 'W2F_PC_Compatibility_Engine' ) ) {
			$engine = W2F_PC_Compatibility_Engine::instance();
			$result = $engine->filter_products_for_component( $component_id, $configuration, $configurator_product );

			// Return all compatible + warning IDs (blocked are excluded).
			return $result['compatible_ids'];
		}
	...
	public function get_product_warnings( $component_id, $product_id, $configuration, $configurator_product = null ) {
		...
		if ( $this->use_new_engine && class_exists( 'W2F_PC_Compatibility_Engine' ) ) {
			$engine = W2F_PC_Compatibility_Engine::instance();
			return $engine->get_product_warnings( $component_id, $product_id, $configuration, $configurator_product );
		}
